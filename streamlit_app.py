...
def plot_power_vectors(S_s, S_i, S_o, S_r, S_loss, Q_s, Q_r):
    ...
    fig.update_layout(
        xaxis=dict(range=[min_x - margin_x, max_x + margin_x], zeroline=True, title="Real Power (MW)"),
        yaxis=dict(range=[min_y - margin_y, max_y + margin_y], zeroline=True, title="Reactive Power (MVAr)"),
...
def main():
    ...
    with col1:
        st.image("https://i.postimg.cc/FKStDhY9/Frame-2-1.png", caption="Transmission Line Pi Section", width=400)
    with col2:
        st.dataframe(df_cases, height=220)

    st.header("Power Diagram and Chart")
