...
# Use wide mode for horizontal space
...
def plot_power_vectors(S_s, S_i, S_o, S_r, S_loss, Q_s, Q_r):
    ...
    # Convert main complex powers to (x,y)
    S_s_xy = to_xy(S_s)
    S_i_xy = to_xy(S_i)
    S_o_xy = to_xy(S_o)
    S_r_xy = to_xy(S_r)
    ...
    # Additional lines
    add_arrow(S_o_xy, S_loss_xy, "magenta", "S_loss")  # from S_o
    add_arrow(S_s_xy, Q_s_xy,    "orange",  "Q_s")     # from S_s
    add_arrow(S_o_xy, Q_r_xy,    "cyan",    "Q_r")     # from S_o
...
def main():
    ...
    # Row 1: image (left), line-case table (right)
