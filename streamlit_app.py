...
def calculate_power_flow(Vs, Vr, R, X, C, delta_deg, base_voltage, length, freq=50):
    ...
    Q_s from tip of S_s -> S_s + jQ_s
    Q_r from tip of S_o -> S_o + jQ_r
    """
...
def plot_power_vectors(S_s, S_i, S_o, S_r, S_loss, Q_s, Q_r):
    ...
    coords = [val for p in all_pts for val in p]
    if any(np.isnan(c) or not np.isfinite(c) for c in coords):
        fig.update_layout(title="Invalid (NaN) results. Check sliders!")
        return fig
...
def main():
    ...
    # Row 1: image on left, table with cases on right
    ...
    with row1_col2:
        ...
        st.dataframe(df_cases.style.format(precision=3), height=250)

    # Row 2: narrower col with case selector + sliders, bigger col with vector diagram
    row2_col1, row2_col2 = st.columns([1,2.2])
    with row2_col1:
    ...
    # Row 3: table with results (left), bar chart (right)
